...
class MedNeXtBlock(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.use_dlka:
            self.dlka = DLKA3D(out_channels)

        self.layerscale = LayerScale(out_channels, dim=dim)
    ...
    def forward(self, x, dummy_tensor=None):
        ...
        # LayerScale after projection
        x1 = self.layerscale(x1)

        # Optional DLKA refinement
        if self.use_dlka:
            identity = x1
            x1 = self.dlka(x1)
            x1 = identity + x1
...
class LayerScale(nn.Module):
    ...
    def __init__(self, channels, init_value=1e-3, dim='3d'):
        super().__init__()

        if dim == '3d':
            self.gamma = nn.Parameter(init_value * torch.ones(channels,1,1,1))
        else:
            self.gamma = nn.Parameter(init_value * torch.ones(channels,1,1))
...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.dec_proj = nn.Conv3d(channels, channels, 1)
    ...
    def forward(self, skip, decoder):
        ...
        alpha = self.attn(skip + decoder)

        return decoder + skip_identity + alpha * skip
